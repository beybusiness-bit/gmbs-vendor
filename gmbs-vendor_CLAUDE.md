...
brands/{brandId}                              — 브랜드회원: 자기 브랜드만 read, 일부 필드 write
  ...
  brand_status, status, biz_no, ceo_name,
  category, phone, email, address, description,
...
firestore.rules     — Firestore 보안 규칙 (customer_inquiries 포함)
vercel.json         — SPA 라우팅
pages/
  brand-info.js     — 브랜드 정보 조회/수정 (brand_code 표시 포함)
  ...
  inquiries.js      — 문의하기
  account.js        — 계정 설정
  customer-inquiries.js — 고객 CS 문의 조회·응답 (신규)
```

**다음 세션에서 이어서 할 수 있는 것들 (우선순위 순):**
- admin 연계 요청사항 목록 정리 후 gmbs-admin 세션에 전달 (customer_inquiries 관리 화면, 담당자 등록 시 login_google_email 분리 등)
- 고객 CS에서 "처리완료" 상태 전환 기능 (현재는 admin만 가능 — vendor도 할 수 있게 할지 확인)
- EmailJS 설정 완료 확인 (admin.gmbs.kr에서 이메일 설정 입력 여부)
- 브랜드 아바타 이미지 업로드 기능 (현재는 이니셜만 표시)
