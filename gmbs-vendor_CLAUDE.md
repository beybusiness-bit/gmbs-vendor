...
**현재까지의 admin 확장 요청 목록** (기획 단계에서 이미 확인된 항목, 상태: 미전달):

1. admin에 "입점 신청함 / 합류 신청함" 화면 추가 — `brand_applications`(신규 브랜드 등록 신청)와 `brand_join_requests`(기존 브랜드 합류 신청) 두 컬렉션을 조회하는 심사 화면. 승인 시 각각 다음을 자동 생성:
   ...
   - 거절 시 상태(거절) + 사유 기록
2. 담당자(Person) 등록 폼에 "연락용 이메일(contact_email)"과 "vendor 로그인용 구글 이메일(login_google_email)"을 별도 필드로 분리해서 입력받고, `login_google_email` 값 기준으로 `vendor_accounts` 문서를 생성·갱신하도록 admin 로직 수정
3. admin의 승인/거절 처리 버튼에 EmailJS 발송 로직 추가 (신청자에게 결과 통보 메일)
4. 담당자 등록 시 이메일(특히 로그인용 구글 이메일) 오타 방지를 위한 확인 입력(더블 체크) 절차 권장
5. Google Cloud OAuth 동의 화면을 "프로덕션(게시됨)" 상태로 전환 필요 — admin·vendor가 같은 프로젝트를 쓰므로 이 설정은 프로젝트 전역에 적용됨. admin 쪽 세션에서도 이 설정 상태를 인지하고 있어야 함(admin의 ALLOWED_EMAILS 코드 레벨 제한은 이 설정과 무관하게 계속 유효함)
...
1단계 프로젝트 셋업(Firebase 연결 — admin과 동일 프로젝트, Google 로그인 연동, 레이아웃 골격) — 🔲 진행 예정
2단계 로그인·계정 생성 로직(로그인/계정만들기 버튼 분리 UX, `users` 자동 생성, `vendor_accounts` 자동 연결 로직) — 🔲 진행 예정
3단계 브랜드 연결 갈래(신규 일반회원 대상 "기존 브랜드 담당자로 연결" vs "새 브랜드 등록" 선택 화면, `brand_join_requests`/`brand_applications` 제출, 본인 신청 현황 조회) — 🔲 진행 예정
4단계 브랜드 정보 조회/수정(브랜드회원) — 🔲 진행 예정
5단계 담당자(Person) 관리(본인 브랜드 범위) — 🔲 진행 예정
6단계 계약 다운로드 — 🔲 진행 예정
7단계 상품 등록/수정요청/상태조회 — 🔲 진행 예정
8단계 재고/판매 조회(읽기전용, gmbs-functions 완성 전까지 "준비중" 처리) — 🔲 진행 예정
9단계 정산 조회(읽기전용) — 🔲 진행 예정
10단계 공지사항·문의 — 🔲 진행 예정
11단계 EmailJS 이메일 연동(신청 접수·처리 알림 등) — 🔲 진행 예정
...
1단계 — 프로젝트 셋업(Firebase 연결 — admin과 동일 프로젝트, Google 로그인 연동, 레이아웃 골격)부터 시작
