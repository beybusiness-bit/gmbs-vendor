...
1. [완료됨] admin에 "입점 신청함 / 합류 신청함" 화면 추가 — `brand_applications`(신규 브랜드 등록 신청)와 `brand_join_requests`(기존 브랜드 합류 신청) 두 컬렉션을 조회하는 심사 화면. 승인 시 각각 다음을 자동 생성:
...
2. [완료됨] 담당자(Person) 등록 폼에 "연락용 이메일(contact_email)"과 "vendor 로그인용 구글 이메일(login_google_email)"을 별도 필드로 분리해서 입력받고, `login_google_email` 값 기준으로 `vendor_accounts` 문서를 생성·갱신하도록 admin 로직 수정
3. [완료됨] admin의 승인/거절 처리 버튼에 EmailJS 발송 로직 추가 (신청자에게 결과 통보 메일). EmailJS 설정은 admin.gmbs.kr → 이메일 설정 메뉴에서 관리.
4. [완료됨] 담당자 등록 시 이메일(특히 로그인용 구글 이메일) 오타 방지를 위한 확인 입력(더블 체크) 절차 권장
5. [완료됨] Google Cloud OAuth 동의 화면을 "프로덕션(게시됨)" 상태로 전환 — 완료
6. [불필요] Firestore 보안 규칙에 `email_configs` 컬렉션 write 권한 — vendor는 읽기만 하며 쓰기 불필요. admin 앱에서만 관리.

7. [미전달] 브랜드 등록 신청 플로우 업데이트 — brand_applications에 brand_type/website_urls/settlement_info/requested_commission_rate 필드 추가됨. admin 심사 화면에 해당 필드 표시 필요. 상세 내용: `docs/admin-handoff-brand-registration.md` 참고 (이 세션에서 전달됨).
   - brands 문서 승인 시 brand_type, fee_info.commission_rate, onboarding_status 입력 필요
   - settlement_info.id_card_url/bank_book_url/biz_reg_url는 admin이 업로드 (vendor 수정 불가)
   - 암호화 키: `app_configs/encryption.aes_key_b64` 에 AES-256 키 저장 필요 (admin과 vendor 동일 키)
...
**추가 구현 (이전 세션 이후 신규):**
- 사이드바 브랜드 헤더 — 원형 아바타 + 브랜드명 + chevron 버튼. 클릭 시 body에 떠오르는 플로팅 팝업 표시 (현재 브랜드 ✓ 표시, 다른 브랜드 전환, 담당 브랜드 관리 버튼)
- 사이드바 라이트 테마 — 배경색 `#1a0533` → `#fff`, 텍스트·아이콘도 밝은 배경에 맞게 전환. 활성 메뉴는 보라색 primary 계열.
- 포인트 컬러 활용 — `badge-green` 배경을 기존 연초록(#dcfce7) → accent 네온 그린(`#9dff20`)으로 교체. 입점확정·승인·처리완료 등 성공 상태에서 강렬하게 표시됨.
- 다중 브랜드 지원 — `getUserBrandIds()`: `brand_ids[]` 배열 우선, 없으면 `brand_id` fallback. `setActiveBrand()`: 활성 브랜드 전환 후 `currentUserDoc.brand_id` 동기화. `getBrandData()`: `brands/{id}`에서 `brand_name`·`brand_photo_url` 조회 + 캐시.
- 어드민 담당자 초대 즉시 반영 버그 수정 — 기존 `users/{uid}` 문서가 있어도 로그인 시 `vendor_accounts`를 확인해서 초대됨 상태이면 해당 브랜드를 `brand_ids`에 추가하고 연결됨으로 전환.
- 담당자 수정/삭제 — 전체 담당자에게 수정 버튼 표시. 본인 외 담당자는 삭제 버튼도 표시 (삭제 확인 모달 포함).
- 고객 문의(CS) 페이지 신설 — `pages/customer-inquiries.js`. `customer_inquiries` 컬렉션 읽기 + `responses` 서브컬렉션. 상태 필터(전체/답변대기/재응답 필요/답변완료/처리완료), 상세 모달, 응답 작성 및 저장. 사이드바 뱃지(미답변 수) 연동.
- 브랜드 코드 표시 — `brand-info.js`의 정보 그리드 상단에 `brand_code` 필드 표시 (admin 입력값, `var(--primary-light)` 배경 코드 태그).
- Firestore 규칙 추가 — `brands/{brandId}/persons` 삭제 허용, `customer_inquiries` 브랜드회원 read + vendor 응답 write + `responses` 서브컬렉션 규칙.
- 사이드바 메뉴 정리 — "브랜드 관리" 섹션 라벨 제거, "브랜드 추가" → "담당 브랜드 관리"로 이름 변경, "담당 브랜드 목록" 메뉴 항목 추가.
- 브랜드 목록 화면 분기 — 브랜드회원: 담당 브랜드 테이블 + 검색 + 클릭으로 전환. 일반회원: 신청 현황 목록 (기존 UX 유지).
...
brand_applications/{id}      // 새 브랜드 등록 신청
  ...
  settlement_info: {
    ...
    resident_number?,                   // 개인 선택 시, "ENC:..." AES-GCM 암호화
...
brands/{brandId}                              — 브랜드회원: 자기 브랜드만 read, 일부 필드 write
  brand_name, brand_code, brand_photo_url,
  brand_type("PB"|"위탁"|"매입"),           // admin 최종 확정 (읽기전용)
  onboarding_status("미계약"|"심사중"|"계약완료"|"승인"|"거절"|"종료"),  // admin 전용 (읽기전용)
  fee_info: { commission_rate: number },    // 위탁 브랜드만, admin 전용 (읽기전용)
  website_urls: string[],                   // vendor 수정 가능
  brand_desc: string,                       // vendor 수정 가능 (구: description)
  settlement_info: {
    business_type, business_reg_number?, taxation_type?,
    resident_number?,                       // "ENC:..." AES-GCM 암호화
    bank_name, account_holder,
    account_number,                         // "ENC:..." AES-GCM 암호화
    id_card_url?, bank_book_url?, biz_reg_url?,  // admin 업로드 (vendor 수정 불가)
  },
  // 구: brand_status, status 필드는 admin도 vendor도 더이상 사용하지 않음 (onboarding_status로 통일)
  biz_no, ceo_name, category, phone, email, address,
  created_at, updated_at
brands/{brandId}/persons/{personId}           — 브랜드회원: 자기 브랜드 범위 read/write/delete
...
customer_inquiries/{docId}                    — admin이 생성(오프라인 매장 접수분), 브랜드회원 read
  brand_id, title, content, type(상품|재입고|불량/고장|기타),
  product_name, sku_id, status(답변대기|재응답 필요|답변완료|처리완료),
  re_response_note, created_at, updated_at
  customer_inquiries/{docId}/responses/{respId}  — 브랜드회원 read+create(author_type=vendor), admin read+create
    content, author_type(vendor|admin), author_email, author_name, created_at
```
...
style.css           — 전체 UI 스타일 (라이트 사이드바, accent 네온 그린 badge-green)
firebase-init.js    — Firebase 초기화 + Firestore 함수 export
app.js              — 인증·라우팅·사이드바(라이트 테마·플로팅 팝업·아바타)·갈래선택·대시보드
emailjs-config.js   — EmailJS 설정 (설정값 입력 필요)
firestore.rules     — Firestore 보안 규칙 (customer_inquiries, app_configs 포함)
vercel.json         — SPA 라우팅
utils/
  encryption.js     — AES-GCM 암호화/복호화 (키: Firestore app_configs/encryption.aes_key_b64)
  validation.js     — 사업자등록번호/주민등록번호 검증 + 포맷터 + 국세청 API
docs/
  admin-handoff-brand-registration.md — admin 연계 요청사항 (브랜드 등록 플로우)
pages/
  brand-info.js     — 브랜드 정보 조회/수정 (onboarding_status, 거래유형, 정산정보, 서류 URL)
  persons.js        — 담당자 관리 (수정·삭제 포함)
  contracts.js      — 계약서 다운로드
  products.js       — 상품 등록/수정요청/상태조회 (에러 핸들링 포함)
  inventory.js      — 재고·판매 조회 (준비중)
  settlements.js    — 정산 조회 (준비중)
  notices.js        — 공지사항
  inquiries.js      — 문의하기
  account.js        — 계정 설정
  customer-inquiries.js — 고객 CS 문의 조회·응답
```

**이번 세션(브랜드 등록 플로우)에서 추가된 파일:**
- utils/encryption.js — AES-GCM 암호화/복호화 (키: Firestore app_configs/encryption.aes_key_b64)
- utils/validation.js — 사업자등록번호/주민등록번호 검증 + 국세청 API 연동
- docs/admin-handoff-brand-registration.md — admin 연계 요청사항 문서

**이번 세션 주요 변경사항:**
- app.js — 브랜드 신청 모달 전면 개편 (brand_type/website_urls/settlement_info/암호화)
- pages/brand-info.js — 전면 재작성 (onboarding_status, 거래유형, 정산정보 섹션, 서류 URL 표시)
- firestore.rules — brands 업데이트 허용 필드 변경 (brand_desc/website_urls/settlement_info/brand_photo_url), settlement_info 서류 URL 필드 벤더 수정 차단, app_configs 읽기 허용
- pages/products.js — getDocs 에러 핸들링 추가 (복합 인덱스 미생성 시 무한로딩 → 에러 메시지 표시)

**다음 세션에서 이어서 할 수 있는 것들 (우선순위 순):**
- Firebase 콘솔에서 products 컬렉션 복합 인덱스 생성 (brand_id ASC + submitted_at DESC) → 상품관리 페이지 무한로딩 해결
- Firebase 콘솔에서 app_configs/encryption 문서에 AES-256 키 등록 (admin과 동일 키 사용)
- admin 연계 요청사항 목록 gmbs-admin 세션에 전달 (docs/admin-handoff-brand-registration.md 참고)
- EmailJS 설정 완료 확인 (admin.gmbs.kr에서 이메일 설정 입력 여부)
