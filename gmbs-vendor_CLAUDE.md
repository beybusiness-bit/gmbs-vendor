...
- pages/account.js — 계정 설정 페이지 (이름/연락처 수정)
...
**전체 구현 완료 (1~11단계 + 추가 기능).** 다음 할 일:

1. **EmailJS 설정** — `emailjs-config.js`의 `publicKey`, `serviceId`, 템플릿 ID를 실제 값으로 교체
   - EmailJS 콘솔(https://www.emailjs.com/) → 서비스·템플릿 생성 후 값 입력
   - 템플릿 변수: `to_email`, `to_name`, `brand_name`
2. **Firestore 보안 규칙 배포** — Firebase 콘솔 → Firestore → 규칙 탭에 `firestore.rules` 내용 붙여넣기
3. **Firebase Authentication 승인 도메인 추가** — 콘솔 → Authentication → 승인된 도메인 → `vendor.gmbs.kr` 추가
4. **OAuth 동의 화면 프로덕션 전환** — Google Cloud Console → API 및 서비스 → OAuth 동의 화면 → 게시 상태 전환
5. **admin 연계 요청 목록 전달** — 이 CLAUDE.md의 "admin 확장 요청 목록"을 admin Claude Code 세션에 전달
