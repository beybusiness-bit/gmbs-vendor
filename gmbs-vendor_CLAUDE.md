...
**전체 구현 완료 (1~11단계 + 추가 기능).** 남은 설정 작업:

1. **EmailJS 설정** ✅ → admin.gmbs.kr 이메일 설정 메뉴에서 UI로 입력 (Firestore `email_configs`에 저장됨)
2. **Firestore 보안 규칙 배포** — Firebase 콘솔 → Firestore → 규칙 탭에 `firestore.rules` 내용 붙여넣기 + `email_configs` 규칙 추가 (아래 참고)
   ```
   ...
   ```
3. **Firebase Authentication 승인 도메인 추가** ✅ — 완료
